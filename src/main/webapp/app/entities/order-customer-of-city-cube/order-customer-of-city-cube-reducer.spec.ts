...
import reducer, {
  ...
  partialUpdateEntity,
  reset,
} from './order-customer-of-city-cube.reducer';
...
describe('Entities reducer tests', () => {
  ...
  describe('Actions', () => {
    ...
    it('dispatches FETCH_ORDERCUSTOMEROFCITYCUBE_LIST actions', async () => {
    ...
    it('dispatches FETCH_ORDERCUSTOMEROFCITYCUBE actions', async () => {
      const expectedActions = [
        {
    ...
    it('dispatches CREATE_ORDERCUSTOMEROFCITYCUBE actions', async () => {
      const expectedActions = [
        {
      ...
      await store.dispatch(createEntity({ cityName: 'CBA' }));
      expect(store.getActions()[0]).toMatchObject(expectedActions[0]);
      expect(store.getActions()[1]).toMatchObject(expectedActions[1]);
      expect(store.getActions()[2]).toMatchObject(expectedActions[2]);
    });

    it('dispatches UPDATE_ORDERCUSTOMEROFCITYCUBE actions', async () => {
      const expectedActions = [
        {
      ...
      await store.dispatch(updateEntity({ cityName: 'CBA' }));
      expect(store.getActions()[0]).toMatchObject(expectedActions[0]);
      expect(store.getActions()[1]).toMatchObject(expectedActions[1]);
      expect(store.getActions()[2]).toMatchObject(expectedActions[2]);
    });

    it('dispatches PARTIAL_UPDATE_ORDERCUSTOMEROFCITYCUBE actions', async () => {
      const expectedActions = [
        {
      ...
      await store.dispatch(partialUpdateEntity({ cityName: 'ABC' }));
      expect(store.getActions()[0]).toMatchObject(expectedActions[0]);
      expect(store.getActions()[1]).toMatchObject(expectedActions[1]);
      expect(store.getActions()[2]).toMatchObject(expectedActions[2]);
    });

    it('dispatches DELETE_ORDERCUSTOMEROFCITYCUBE actions', async () => {
      const expectedActions = [
        {
